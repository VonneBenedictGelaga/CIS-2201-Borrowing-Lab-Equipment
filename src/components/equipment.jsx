...
const RequestBorrower = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    // Check if the date of borrowing is today or in the past
    const today = new Date().toISOString().split('T')[0];
    if (dateBorrow <= today) {
      console.log('Invalid date of borrowing. Date must be in the future.');
      return;
    }
    ...
    try {
      ...
      // Check if there is an ongoing request with the same borrower's email
      const requestRef = collection(db, 'Request');
      const requestSnapshot = await getDocs(requestRef);
      const ongoingRequest = requestSnapshot.docs.some(
        (doc) =>
          doc.data().borrowerEmail === borrowersEmail &&
          doc.data().status3 !== 'Returned'
      );
      ...
      {invalidRequest && (
        <div className="overlay">
          <div className="invalid-modal">
            <div className="invalid-modal-content">
              <h3>Invalid Request</h3>
              <p>An ongoing request with the same borrower's email exists.</p>
              <button className="btn btn-primary" onClick={() => setInvalidRequest(false)}>
                OK
              </button>
            </div>
          </div>
